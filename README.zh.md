...
安装完成后和每次自动启动前，脚本都会先检查 Codex 是否已经是最新版本；不是最新版才更新，并且还会检查订阅是否快到期或已到期，只提示，不阻断。它还会在每天首次启动前静默刷新本地 skills 和 plugins，并按 manifest 恢复当前已记录的 Claude plugins 状态。在线一键入口现在默认无人值守运行，不会停在确认问题上，也不会自动创建 Linux 用户；如果检测不到普通用户，只会打开一次 WSL 让你手动创建。
